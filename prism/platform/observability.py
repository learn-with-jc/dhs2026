# prism/platform/observability.py
"""
...
Both share a single SQLite file: data/processed/prism.db
"""
...
DB_PATH = Path(__file__).resolve().parents[2] / "data" / "processed" / "prism.db"
...
class LocalSQLiteTracer(BaseCallbackHandler):
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        # (LangGraph/LCEL sometimes invoke this callback with serialized=None)
...
def setup_tracing() -> list[LocalSQLiteTracer]:
    ...
    Local SQLite path (returns [LocalSQLiteTracer()]):
      ...
      data/processed/prism.db :: trace_runs table.
    """
    import os
